...
- **const 正确性**：在 const 成员函数中，`pimpl` 指针本身是 const 的（不能指向其他对象），但它指向的数据默认是可修改的。这可能破坏 const 承诺。
    - C++20 引入的 `std::experimental::propagate_const` 可以帮助解决这个问题，它能将外层指针的 const 属性传播到其管理的指针。

## 总结：

PIMPL 模式是一个非常实用的设计模式，尤其适用于以下场景：
