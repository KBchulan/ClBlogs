...
description: 基本数据类型、语句与表达式、函数相关内容介绍
...
Rust 使用一个相对传统的语法来创建整数和浮点数。整数、浮点数的运算和你在其它语言上见过的一致，都是通过常见的运算符来完成。

> 不仅仅是数值类型，Rust 也允许在复杂类型上定义运算符，例如在自定义类型上定义 `+` 运算符，这种行为被称为运算符重载，Rust 具体支持的可重载运算符见[附录 B](https://kbchulan.github.io/ClBlogs/blogs-main/rust/02-else-rust.html)。
...
这些语句中的每个表达式都使用了数学运算符，并且计算结果为一个值，然后绑定到一个变量上。[附录 B](https://kbchulan.github.io/ClBlogs/blogs-main/rust/02-else-rust.html) 中给出了 Rust 提供的所有运算符的列表。
...
该函数如此简单，但是又是如此的五脏俱全，声明函数的关键字 `fn`，函数名 `add()`，参数 `i` 和 `j`，参数类型和返回值类型都是 `i32`，和其他语言都是一样的。

### 函数要点

- 函数名和变量名使用 **蛇形命名法**，例如 `fn add_two() {}`
- 函数的位置可以随便放，Rust 不关心我们在哪里定义了函数，只要有定义即可
- 每个函数参数都需要标注类型，返回类型也需要标注类型
...
注意，能表达发散的是 loop 或 panic，`while true` 不行，毕竟这个语句是可以退出的，因此他返回的是一个()。

至此，我们介绍了rust中的基础类型，到目前为止一切顺利，没有什么难点，接下来将是不一样的 rust，逐步探讨此语言的高级特性。
