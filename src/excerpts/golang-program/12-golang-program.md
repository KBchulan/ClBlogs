...
![](/assets/pages/golang-program/12-1.png)
...
*   <p>URI结尾不应包含`/`。</p>
*   <p>URI中不能出现下划线 `_`，必须用中杠线 `-`代替（有些人推荐用 `_`，有些人推荐用 `-`，统一使用一种格式即可，我比较推荐用 `-`）。</p>
*   <p>URI路径用小写，不要用大写。</p>
*   <p>避免层级过深的URI。超过2层的资源嵌套会很乱，建议将其他资源转化为`?`参数，比如：</p>
...
![](/assets/pages/golang-program/12-2.png)
...
![](/assets/pages/golang-program/12-3.png)
