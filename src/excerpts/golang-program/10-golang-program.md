...
第一个手段是，组织一个好的目录结构。关于如何组合一个好的目录结构，你可以回顾 [06讲](https://kbchulan.github.io/ClBlogs/excerpts/golang-program/06-golang-program.html) 的内容。
...
在BirdInfo函数中，将Birds接口类型作为参数传入，并在函数中调用了`birds.Name`，`birds.Class`方法，这两个方法会根据birds类别的不同而返回不同的名字和类别，也就是说通过接口实现了多态。
...
![](/assets/pages/golang-program/10-9.png)
...
![](/assets/pages/golang-program/10-12.png)
...
## 编写高质量的项目文档
最后，一个优雅的项目，还应该有完善的文档。例如 README.md、安装文档、开发文档、使用文档、API接口文档、设计文档等等。这些内容在[第04讲](https://kbchulan.github.io/ClBlogs/excerpts/golang-program/04-golang-program.html)的文档规范部分有详细介绍，你可以去复习下。
