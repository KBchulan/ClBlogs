...
在 [**08讲**](https://kbchulan.github.io/ClBlogs/excerpts/golang-program/08-golang-program.html) 和 [**14讲**](https://kbchulan.github.io/ClBlogs/excerpts/golang-program/14-golang-program.html) ，我分别介绍了如何设计研发流程，和如何基于 Makefile 高效地管理项目。那么今天，我们就以研发流程为主线，来看下IAM项目是如何通过Makefile来高效管理项目的。学完这一讲，你不仅能更加深刻地理解 **08讲** 和 **14讲** 所介绍的内容，还能得到很多可以直接用在实际操作中的经验、技巧。
...
![](/assets/pages/golang-program/15-1.png)
...
这些操作中包含了low code的思想。在第 [**10讲**](https://kbchulan.github.io/ClBlogs/excerpts/golang-program/10-golang-program.html) 中我就强调过，要尽可能使用代码自动生成这一技术。这样做有两个好处：一方面能够提高我们的代码开发效率；另一方面也能够保证规范，减少手动操作可能带来的错误。所以这里，我将iamctl的命令也模板化，并通过 `iamctl new` 自动生成。
...
这里还有个Makefile编写技巧：如果Makefile的command需要某个命令，就可以使该目标依赖类似tools.verify.addlicense这种目标，tools.verify.addlicense会检查该工具是否已安装，如果没有安装则自动安装。
...
![](/assets/pages/golang-program/15-2.png)
...
![](/assets/pages/golang-program/15-3.png)
...
![](/assets/pages/golang-program/15-4.png)
...
![](/assets/pages/golang-program/15-5.png)
...
![](/assets/pages/golang-program/15-6.png)
