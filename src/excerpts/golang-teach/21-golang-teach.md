---
title: 20 协作开发：模块化管理为什么能够提升研发效能？
article: true
...
除了 main 这个特殊的函数外，Go 語言還有一個特殊的函數——init，通過它可以**實現包級別的一些初始化操作**。

init 函數沒有返回值，也沒有參數，它**先於 main 函數執行**，代碼如下所示：
```go
func init() {
